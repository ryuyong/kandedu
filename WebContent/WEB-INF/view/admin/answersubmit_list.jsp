...
function makeexcel() {
	var firstsearch = '${AnswerVo.firstsearch}';
	if(firstsearch != '1') {
		alert('검색조건을 선택후 검색해주세요');
		return;
	}
	...
	if(confirm("엑셀을 생성하시겠습니까?")) {
		...
		frm_insert.dayvalue.value = dayvalue;		
...
<form class="form-inline" id="frm_search" name="frm_search" method="post" action="<c:url value="/admin/answersubmit_list.do" />">
...
<div class="row">
	<div class="container col-xs-12">
		<table class="table table-bordered table-striped">
			<colgroup><col width="20%" /><col width="10%" /><col width="10%" /><col width="10%" /><col width="10%" /><col width="10%" /><col width="10%" /><col width="10%" /><col width="10%" /></colgroup>
			<thead>
				<tr>
					<th>학원명</th>
					<th>과목</th>
			...
			<tbody>
				<c:if test="${empty answersubmitststsList}">
					<tr>
						<c:if test="${empty AnswerVo.firstsearch}">
				...
				<c:if test="${!empty answersubmitststsList}">
					<c:forEach var="answersubmitststsList" items="${answersubmitststsList}" varStatus="status">
						<tr>
							<td>${answersubmitststsList.hakwon_nm}</td>
							...
							<td>
								<c:if test="${!empty answersubmitststsList.submitcnt}">
									<a class="btn btn-warning" href="javascript:deleteanswer('${answersubmitststsList.stud_cd}','${answersubmitststsList.kwamok_cd}' ,'${answersubmitststsList.weekvalue}','${answersubmitststsList.dayvalue}')">삭제</a>
								</c:if>
								<c:if test="${empty answersubmitststsList.submitcnt}">
									&nbsp;
								</c:if>
...
<div class="row">
	<div class="container col-xs-12">
		<a class="btn btn-primary" href="javascript:makeexcel()" role="button">엑셀파일생성</a>
	</div>
</div>
...
<form id="frm_insert" name="frm_insert" method="post" action="<c:url value="/admin/answersubmit_aftermake.do" />">
	<input type="hidden" name="hakwon_cd" value="" />
	<input type="hidden" name="hakneon" value="" />
	<input type="hidden" name="kwamok_cd" value="" />
	<input type="hidden" name="weekvalue" value="" />
	<input type="hidden" name="dayvalue" value="" />
</form>